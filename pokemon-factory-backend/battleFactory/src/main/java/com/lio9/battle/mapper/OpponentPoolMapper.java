...
public interface OpponentPoolMapper {
    ...
    @Select("SELECT op.id, t.team_json, op.rank FROM opponent_pool op JOIN team t ON op.team_id = t.id WHERE op.rank BETWEEN #{low} AND #{high} ORDER BY RANDOM() LIMIT #{limit}")
    List<Map<String,Object>> sample(@Param("low") int low, @Param("high") int high, @Param("limit") int limit);
