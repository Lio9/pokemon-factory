...
基于 **Three.js** 的全新 3D 对战体验，提供沉浸式战斗场景和实时动画反馈。

**访问路径**: `/battle3d`

**核心特性**:
- **3D 战场环境**：椭圆形草地平台，玩家/对手阵营划分，装饰物（花朵、岩石）
- **宝可梦 3D 模型**：几何体组合占位，支持 18 种属性颜色映射
- **实时动画反馈**：攻击冲刺、受击抖动、治愈光晕、倒下消散
- **粒子特效系统**：攻击命中、属性特效、太晶化光芒、相机震动
- **混合交互模式**：点击选中 + 键盘快捷键（1-4 招式、Q 换人、E 特殊系统）
- **调试面板**：实时 FPS、战斗阶段、日志输出
...
cd frontend
npm run dev
# 访问 http://localhost:7894/battle3d
```

**详细设计文档**: [docs/battle3d-design.md](docs/battle3d-design.md)
