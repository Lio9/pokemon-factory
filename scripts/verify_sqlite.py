...
def verify_data():
    ...
    cursor.execute("SELECT COUNT(*) FROM move_meta")
    ...
    cursor.execute("SELECT COUNT(*) FROM pokemon_form")
    ...
    cursor.execute("SELECT COUNT(*) FROM type_efficacy")
    efficacy_count = cursor.fetchone()[0]
    
    expected_efficacy = type_count * type_count
    print(f"  属性数量: {type_count}")
    print(f"  属性相性记录: {efficacy_count:,}")
    print(f"  预期记录数: {expected_efficacy:,}")
    print(f"  完整性: {efficacy_count/expected_efficacy*100:.1f}%")
    print()
    
    # 验证进化数据
    print("🔍 进化数据验证:")
    print("-" * 60)
    cursor.execute("SELECT COUNT(*) FROM evolution_chain")
    chain_count = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM pokemon_evolution")
