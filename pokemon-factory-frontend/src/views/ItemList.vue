<template>
  <div ref="listContainer" class="item-list">
    <!-- 搜索栏 -->
    <div class="glass-card mb-6 p-4 sticky top-[4.25rem] z-10 sm:top-[4.75rem]">
      <div class="flex flex-col sm:flex-row gap-4">
        <div class="flex-1">
          ...
          >
            <template #append>
              <el-button class="!bg-gradient-to-r !from-indigo-500 !to-purple-600 !text-white !border-none hover:!from-indigo-600 hover:!to-purple-700" @click="handleSearch">
    ...
    <!-- 加载骨架 -->
    <div v-if="loading && items.length === 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
    ...
    <div v-else class="text-center py-16">
      ...
      <p class="text-slate-500">{{ tr('没有找到物品', 'No items found') }}</p>
    </div>
    ...
    <div ref="loadMoreTrigger" class="text-center py-8">
      ...
      <div v-else-if="!hasMore && items.length > 0" class="text-sm text-slate-400">
        {{ tr('已加载全部 {total} 个物品', 'All {total} items loaded', { total }) }}
      </div>
      <div v-else-if="!hasMore" class="text-sm text-slate-400">{{ tr('下拉加载更多...', 'Scroll to load more...') }}</div>
    </div>
  </div>
</template>
