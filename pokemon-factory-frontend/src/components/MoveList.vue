<template>
  <div class="move-list" ref="listContainer">
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 mb-6 sticky top-0 z-10">
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <div class="w-32">
            <el-select v-model="selectedDamageClass" placeholder="分类" clearable @change="handleSearch">
              <el-option label="物理" value="physical" />
              <el-option label="特殊" value="special" />
              <el-option label="变化" value="status" />
            </el-select>
          </div>
          <el-button
        ...
        <el-collapse-transition>
          <div v-if="showFilters" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-[150px]">
              ...
              <el-select v-model="powerRange" placeholder="威力" clearable @change="handleSearch">
                <el-option label="无威力" value="none" />
            ...
            <div class="flex-1 min-w-[150px]">
              ...
              <el-select v-model="accuracyRange" placeholder="命中" clearable @change="handleSearch">
                <el-option label="必中" value="100" />
                <el-option label="90-99" value="90-99" />
            ...
            <div class="flex-1 min-w-[150px]">
              ...
              <el-select v-model="ppRange" placeholder="PP" clearable @change="handleSearch">
            ...
            <div class="flex-1 min-w-[150px]">
              ...
              <el-select v-model="sortBy" @change="handleSort">
                ...
                <el-option label="威力 高→低" value="power-desc" />
                <el-option label="威力 低→高" value="power-asc" />
                <el-option label="命中 高→低" value="accuracy-desc" />
                <el-option label="命中 低→高" value="accuracy-asc" />
                <el-option label="PP 高→低" value="pp-desc" />
                <el-option label="PP 低→高" value="pp-asc" />
        ...
        <el-skeleton v-for="i in 6" :key="i" animated>
          <template #template>
            <el-card class="mb-4">
              ...
              <el-skeleton-item variant="rect" style="width: 100%; height: 60px; margin-top: 10px" />
            </el-card>
          </template>
        </el-skeleton>
      </div>
    ...
    <!-- 网格视图 -->
    <div v-else-if="moves.length && viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div
        v-for="move in moves"
        :key="move.id"
      ...
      >
        ...
        <div class="grid grid-cols-3 gap-2 text-center text-sm">
          <div class="bg-red-50 rounded-lg py-2">
            <div class="font-semibold text-red-600">{{ move.power || '-' }}</div>
            <div class="text-xs text-red-400">威力</div>
          </div>
          <div class="bg-blue-50 rounded-lg py-2">
            <div class="font-semibold text-blue-600">{{ move.accuracy || '-' }}</div>
            <div class="text-xs text-blue-400">命中</div>
          </div>
          <div class="bg-green-50 rounded-lg py-2">
            <div class="font-semibold text-green-600">{{ move.pp || '-' }}</div>
            <div class="text-xs text-green-400">PP</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div v-if="selectedMove">
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
          <div class="text-center">
            <div class="text-2xl font-bold text-red-600">{{ selectedMove.power || '-' }}</div>
            <div class="text-sm text-gray-500">威力</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-blue-600">{{ selectedMove.accuracy || '-' }}</div>
            <div class="text-sm text-gray-500">命中</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-green-600">{{ selectedMove.pp || '-' }}</div>
            <div class="text-sm text-gray-500">PP</div>
          </div>
        </div>
