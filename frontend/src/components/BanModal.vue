    ...
    <div class="relative w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <div class="bg-gradient-to-r from-rose-600 to-pink-600 px-6 py-4">
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-lg font-bold text-white">🚫 Ban 宝可梦</h2>
            <p class="text-sm text-white/80">选择不想在对手队伍中遇到的宝可梦</p>
          </div>
          <button @click="$emit('close')" class="rounded-lg p-2 text-white/80 hover:bg-white/20 hover:text-white transition">
      ...
      <div class="border-b border-slate-200 bg-slate-50 px-6 py-3">
        <div class="flex items-center justify-between">
          ...
          <div class="text-sm text-slate-600">
            本次费用：<span class="font-bold" :class="totalCost > 0 ? 'text-amber-600' : 'text-emerald-600'">{{ totalCost > 0 ? totalCost + ' 积分' : '免费' }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="border-b border-slate-200 px-6 py-4">
        <div class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Ban 槽位</div>
        <div class="grid grid-cols-3 sm:grid-cols-6 gap-2">
          <div v-for="slot in slots" :key="slot.index"
            ...
            <div class="text-lg mb-1">{{ slot.banned ? '🚫' : '➕' }}</div>
            <div class="text-[10px] font-bold text-center">
              {{ slot.banned ? slot.pokemonName : `第${slot.index +1}只` }}
            </div>
            <div v-if="slot.cost > 0" class="mt-1 text-[10px] px-1.5 py-0.5 rounded-full"
              :class="slot.affordable ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'">
              {{ slot.cost }}分
            </div>
            <div v-else class="mt-1 text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
              免费
            </div>
          </div>
        </div>
      ...
      <!-- 宝可梦选择列表 -->
      <div class="overflow-y-auto px-6 py-4" style="max-height: 40vh;">
        <div class="mb-3 flex items-center gap-2">
          <input v-model="searchQuery" type="text" placeholder="搜索宝可梦..."
          ...
          <select v-model="rarityFilter" class="rounded-lg border border-slate-300 px-3 py-2 text-sm">
            <option value="">全部稀有度</option>
            <option value="common">普通</option>
            <option value="rare">稀有</option>
            <option value="epic">史话</option>
            <option value="legend">传说</option>
          </select>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          <button v-for="pokemon in filteredPokemon" :key="pokemon.name_en"
            ...
            <img :src="getSpriteUrl(pokemon)" class="h-8 w-8 object-contain" @error="$event.target.src='/placeholder.png'">
            <div class="flex-1 min-w-0">
              <div class="font-medium text-slate-800 truncate">{{ pokemon.name || pokemon.name_en }}</div>
              <div class="text-[10px] text-slate-400">#{{ pokemon.id }}</div>
            </div>
            <div v-if="isBanned(pokemon)" class="text-rose-500 text-xs font-bold">已Ban</div>
          </button>
        </div>
      </div>
      ...
      <div class="border-t border-slate-200 bg-slate-50 px-6 py-4">
        <div class="flex items-center justify-between">
          <button @click="clearAllBans" class="text-sm text-slate-500 hover:text-slate-700 transition">
            清空所有 Ban
          </button>
          <div class="flex gap-2">
            <button @click="$emit('close')" class="rounded-lg px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-200 transition">
              取消
            </button>
            <button @click="confirmBans"
