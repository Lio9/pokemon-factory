    ...
    <Transition name="modal">
      ...
      >
        ...
        <div class="relative bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          <button
            class="absolute top-4 right-4 w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"
            @click="$emit('close')"
          >
...
const CompareView = {
  ...
  template: `
    <div class="grid md:grid-cols-2 gap-8">
      ...
      <div v-else class="flex items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl">
        <button
